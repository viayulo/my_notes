...
Show tag about why ability activation fails.

Setup in editor or override ~InitGlobalTags()~ in your custom ~AbilitySystemGlobal~.

#+begin_src cpp
void UExtendedAbilitySystemGlobals::InitGlobalTags()
{
    ActivateFailIsDeadTag = GameplayAbilitiesExtensionTags::Ability_ActivateFail_IsDead;
    ActivateFailCooldownTag = GameplayAbilitiesExtensionTags::Ability_ActivateFail_Cooldown;
    ActivateFailCostTag = GameplayAbilitiesExtensionTags::Ability_ActivateFail_Cost;
    ActivateFailTagsBlockedTag = GameplayAbilitiesExtensionTags::Ability_ActivateFail_TagsBlocked;
    ActivateFailTagsMissingTag = GameplayAbilitiesExtensionTags::Ability_ActivateFail_TagsMissing;
    ActivateFailNetworkingTag = GameplayAbilitiesExtensionTags::Ability_ActivateFail_Networking;
...
Need to hard-coded ~FGuid~ for data, have enough bits that the likelihood of a duplicate is so incredibly unlikely that you don't have to worry about it as long as you _aren't following an obvious pattern_.
...
Each StateTree asset saves the schema class name in asset data tags, which can be used to limit which StatTree assets can be selected per use case, i.e.:

#+begin_src cpp
UPROPERTY(EditDefaultsOnly, Category = AI, meta=(RequiredAssetDataTags="Schema=/Script/Module.StateTreeSchema_SupaDupa"))
...
In new version, parameters moved to instanced data.

schema limited by: ~meta = (Schema = "/Script/Module.StateTreeSchema_SupaDupa")~

**** Parameters (~FInstancedPropertyBag~)

Default (static) value stored in ~UStateTree~ asset, and get via ~GetDefaultParameters()~.

By default, runtime global parameters of StateTree stored in =InstanceStorage=, and only set once at start.

You can also use external global parameters.

if set a new tree to ~FStateTreeReference~ via ~SetStateTree(InStateTree)~, parameters will migrate to parameters of new tree by ~SyncParameters()~ (if ID of parameters in old tree also exist in new tree, and in ~PropertyOverrides~ array, the value will get migrated).

***** External Global Parameters

Override ~GetGlobalParameterDataType()~ in StateTreeSchema to change data type the global parameter type uses.

#+begin_src cpp
virtual EStateTreeParameterDataType GetGlobalParameterDataType() const override { return EStateTreeParameterDataType::ExternalGlobalParameterData; }
#+end_src

**** Instance Data (~FStateTreeInstanceData~ wraps ~FStateTreeInstanceStorage~)

Store the runtime state of a =StateTree=, interact with data via =Execution Context=.
...
Serialization is supported only for ~FArchive::IsModifyingWeakAndStrongReferences()~, that is replacing object references.

***** ~FStateTreeExecutionState~ (Stored inside =InstanceStorage=)

Describes the execution state of the current State Tree instance.

~Context.GetExecState()~

****** ~FStateTreeExecutionFrame~

Describes an active branch of a specific State Tree that is running.

~Context.GetActiveFrames()~ (allow ReadOnly/Minimal EC)

Has unique =FrameID= (~UE::StateTree::FActiveFrameID~, uint32 wrapper).

Most time you only has 1 frame per execution, but if multiple subtrees used, there might be multiple.

init frame has no active state, and has ~bIsGlobalFrame~ set to true (the global tasks of the State Tree should be handle).

multiple active frames

RootFrame is ActiveFrames[0]
...
Describes an array of active states (~FStateTreeStateHandle~ and its ID) in a State Tree.

Max active state is 8, so depth of child state is 8.

******** ~FStateTreeStateHandle~

Has unique =StateID= (~UE::StateTree::FActiveStateID~, uint32 wrapper), ID array stored in ~FStateTreeActiveStates~.

Get state by ~UStateTree::GetStateFromHandle(Handle)~.

Handle of root state is ~FStateTreeStateHandle::Root~ (~FStateTreeStateHandle(0)~).

***** Data Handle (~FStateTreeDataHandle~)

Handle allows frame relative access of instance data

***** Usage

****** Init

~Context.Start()~ will reset the =InstanceData=
...
if (SetContextRequirements(Context))
{
    ...
    FStateTreeFooBarExecutionExtension Extension;
    ...
    const EStateTreeRunStatus CurrentRunStatus = Context.Start(FStateTreeExecutionContext::FStartParameters{
        ...
        .ExecutionExtension = TInstancedStruct<FStateTreeFooBarExecutionExtension>::Make(MoveTemp(Extension))
    });
...
~TickUpdateTasks(DeltaTime)~ updates the tasks first, and then ~TickTriggerTransitions()~ triggers the transitions.
~Tick(DeltaTime)~ will do both.
...
**** StateTree Data Version

~FStateTreeCustomVersion::Type~ in =StateTree.h=

Version bump requires recompilation of StateTree assets.

*** Execution Context

StateTree Execution Context is a helper that is used to update StateTree instance data.

Each time of update create a new EC and run with it.
...
#+end_quote
You can only tick on demand, like mass signal subsystem, and the tick delta time might be long.
...
Information on how a state tree should tick next. A wrapper for ~NextDeltaTime~.

Get by ~Context.GetNextScheduledTick()~, pass by value.

| Schedule                  | Description                                                 |
|---------------------------+-------------------------------------------------------------|
| ShouldSleep()             | Doesn't need to tick until an event/delegate/transition/... |
| ShouldTickEveryFrames()   | Tick every frame.                                           |
| ShouldTickOnceNextFrame() | Tick once next frame.                                       |
| HasCustomTickRate()       | Has a custom tick rate (~GetTickRate()~).                     |

When ~GetNextScheduledTick()~ get called, a ~FStateTreeScheduledTick~ made by EC depends on running state and settings.

| Made by                       | Value of ~NextDeltaTime~     | Condition                                          |
|-------------------------------+----------------------------+----------------------------------------------------|
| MakeSleep()                   | >= ~UE_FLOAT_NON_FRACTIONAL~ | TreeRunStatus != =Running=, or context is not valid. |
| MakeEveryFrames()             | 0                          | not IsScheduledTickAllowed() in schema             |
| MakeNextFrame()               | ~UE_KINDA_SMALL_NUMBER~      |                                                    |
| MakeCustomTickRate(DeltaTime) | > 0                        |                                                    |

***** ~FStateTreeExecutionExtension~

Used by the execution context or a weak execution context to extend their functionalities.

Stored in, and pass to

****** Usage

create struct inherit ~FStateTreeExecutionExtension~, add your own data.
#+begin_src
...
Usually returned by ~Context.GetStateTreeRunStatus()~, ~Start()~, ~Tick()~, ~Stop()~.
...
Allow tasks to request transitions
...
- ~Category = "Context"~ :: Bind to context or parameters (bind automatically/randomly if not assigned)
...
**** External Data (~TStateTreeExternalDataHandle<>~)
...
| Node Type | Eval       | Eval      | Eval           | Eval           |
| Task      | EnterState | Tick      | ExitState      | StateCompleted |
| Condition | EnterState | ExitState | StateCompleted |                |

- Reselect :: (Default) condition will receive EnterState/ExitState even if the state was previously active.
...
****** Setup custom condition

- (Recommended) create a base struct for schemas to safely include.

#+begin_src cpp
USTRUCT(Category="Foo|Bar", meta = (Hidden))
struct FFooBarStateTreeConditionBase : public FStateTreeConditionBase
{
    GENERATED_BODY()
};
#+end_src

- Add InstanceData if needed.
- override ~TestCondition()~.
- override ~GetDescription()~ to describe condition in editor

#+begin_src cpp
USTRUCT()
struct FFooBarStateTreeCondition_Foo_InstanceData
{
    GENERATED_BODY()
...
STATETREE_POD_INSTANCEDATA(FFooBarStateTreeCondition_Foo_InstanceData);

USTRUCT(DisplayName="Foo Bar", Category="Foo|Bar")
struct FFooBarStateTreeCondition_Foo : public FFooBarStateTreeConditionBase
{
    GENERATED_BODY()

    using FInstanceDataType = FFooBarStateTreeCondition_Foo_InstanceData;

    FFooBarStateTreeCondition_Foo = default;
    explicit FStateTreeCompareIntCondition(const EStateTreeCompare InInverts = EStateTreeCompare::Default)
    ...
    // If true, the comparison result will be inverted (i.e., "not equal" instead of "equal")
    UPROPERTY(EditAnywhere, Category = "Parameter")
    bool bInvert = false;
};
#+end_src
...
#if WITH_EDITOR
FText FFooBarStateTreeCondition_Foo::GetDescription(const FGuid& ID, FStateTreeDataView InstanceDataView, const IStateTreeBindingLookup& BindingLookup, EStateTreeNodeFormatting Formatting) const
{
    ...
    const FText Format = (Formatting == EStateTreeNodeFormatting::RichText)
        ? LOCTEXT("FooConditionRich", "{EmptyOrNot}{Left} <s>is</> {Right}")
        : LOCTEXT("FooCondition", "{EmptyOrNot}{Left} is {Right}");
...
The condition instance data is shared between all the uses a State Tree asset.
You should not modify the instance data in ~EnterState()~, ~ExitState()~ and ~StateCompleted()~ callback.

Use ~STATETREE_POD_INSTANCEDATA()~ macro to define runtime data for faster init.
POD means =Plain Old Data= (i.e. no pointers etc.), has no user-defined destructor, no user-defined copy assignment operator, and no nonstatic members of pointer-to-member type.
In C++ pointers are POD, but UE doesn't consider them as POD.
POD allocated at once, initialized with zeros, and that's it, no constructors are being called. And the whole memory pool is released at once, no destructors are being called for individual memory "cells".
unlike BT instance memory is one big pool for all nodes, only PODs are really safe to be used, otherwise you risk memory leaks; you can use non-POD data in InstanceData of condition.
...
A way for a plugin to add extra information to the StateTree. They are not part of the binding system. Add a PostInternalCompilation step; extensions can be added in that step.
...
** Animation Blueprint (Legacy)

+ Parent Native Class :: ~UAnimInstance~

#+begin_quote
it's too easy for ABP to fall on the slow path and become game thread synchronous-- even just setting root motion on everything silently makes it unthreaded (it makes sense why, its just such an easy trap)
#+end_quote
...
SoundCues do their graph evaluation on the Audio Thread, which is lock-stepped with the Game Thread. A heavy SoundCue or a lot of them can impact the Game Thread by stalling GC.
MetaSounds are rendered in tasks asynchronous to the game thread, so if they run slow, it doesn't impact the game thread, it impacts the audio render.
There's no difference between a Sound Cue playing 2 sound waves and a Blueprint playing 2 sound waves.
Except where the logic is evaluated.
BP on the Game Thread, SC on the Audio Thread.
#+end_quote
...
Then there's the issue Dan was talking about with playing back mono files in the content browser and comparing it to external tools (wave editors, DAWs).  Mono assets in UE have an option in how you want to "upmix" it to stereo (it'd defined in the project settings). Technically, when you play a mono file, to do it accurately, you'd only hear it in the left channel. Nobody wants that. So you gotta "upmix" it to stereo. The problem there is there's a couple ways to go about that. You can do "dual mono" (which is how most wave editors do it) -- basically just dupe the audio in both left and right channels. This makes sense in the context of a DAW or a wave editor. But, in the context of a game engine, it may make more sense to try to align your mono upmix method to match what the sound would be if it was played in the engine and panned to the center without attenuation. Note that this is basically so you can make consistent mix choices about the volume of your sounds independently of if it's a 2D sound or a 3D sound. It's reasonable to consider a 2D mono sound to be equivalent to a center-panned non-attenuated 3D sound.

Thus you have the option of choosing "linear" and "equal power" in addition to dual-mono. These are the same options avaialble in the project settings for choosing your panning method. Linear panning would be to put 50% of the channel in left and 50% of the channel in the right. Equal power would put ~70% in the left and ~70% in the right. Note that linear and equal power have interesting consequences for panning.  Linear panning has a -3 dB drop off in perceived volume when panned to the middle. Thus, a "linear mono upmix method" will effectively add a -3 dB attenuation when you play the mono asset in 2D compared to playing it without an upmix (i.e. out of the left channel). Equal power compensates for that by balancing the square of the amplitude (which is power) so that it's constant.  Dual mono is going to be 2x the amplitude of the single channel asset.

Side note: in a surround sound context, with linear panning, if you pan a mono sound evenly around the listener, as it traverses over speakers, you can hear it get louder as it gets closer to the speaker and quieter (-3 dB) as it gets between speakers. This can be percieved by some as a "widening" of the field and some games like this sound. It kind of exaggerates the edges of the field. Equal power panning, on the other hand, is percieved as constant volume (because volume perception is based on the power of the signal, not the amplitude) as it traverses around the listener.  IMO, linear panning is fundamentally inferior for a number of reasons, chief of which is inconsistencies when mixing the game. It's hard to predict volumes when it's angular position can have such a big volume delta.
#+end_quote
...
|                               | On Dedicated Server | On Client 1      | On Client 2      |
|-------------------------------+---------------------+------------------+------------------|
| Client 1 Character =Role=       | Authority           | Autonomous Proxy | Simulated Proxy  |
| Client 1 Character =RemoteRole= | Autonomous Proxy    | Authority        | Authority        |
|-------------------------------+---------------------+------------------+------------------|
| Client 2 Character =Role=       | Authority           | Simulated Proxy  | Autonomous Proxy |
| Client 2 Character =RemoteRole= | Autonomous Proxy    | Authority        | Authority        |
...
**** Step 2-5: You MUST wrap your TArray in another struct that inherits from =FFastArraySerializer=
...
| Class   | Bytes | Case        | Mutability             | Localization  | Usage                              |
|---------+-------+-------------+------------------------+---------------+------------------------------------|
| =FString= |    40 | Sensitive   | Mutable                | Not localized | ?                                  |
| =FText=   |    16 | Sensitive   | Immutable, formattable | Localized     | for all text shown to players (UI) |
| =FName=   |     4 | Insensitive | Immutable              | Not localized | for IDs (Names)                    |
...
| Conversions | To =FString=  | To =FText=                      | To =FName=           |
|-------------+-------------+-------------------------------+--------------------|
| =FString=     | -           | ~FText::FromString(YourString)~ | ~FName(*YourString)~ |
| =FText=       | ~.ToString()~ | -                             | -                  |
| =FName=       | ~.ToString()~ | ~FText::FromName(YourName)~     | -                  |
...
- after 5.7, FString ctor don't need TEXT macro either.
...
A bag of parameters with any type, naming. (use case: StateTree Parameters, Chooser, etc.)

*** ~FInstancedPropertyBag~

Hold a InstanceStruct of ~UPropertyBag~, type of ~UPropertyBag~ struct based on layout of property (~FPropertyBagPropertyDesc~).

**** Usage

#+begin_src cpp
UPROERTY(EditDefaultsOnly, Category = Common) // if only value is editable in editor, use meta = (FixedLayout)
...
// Adding or removing properties is not cheap,
// each add/remove call allocate memory for new UPropertyBag,
// so better do it in batches.
Bag.AddProperties({
...
// For container type property
// Array
static const FName FloatArrayName(TEXT("Array"));
Bag.AddProperties({
    { FloatArrayName, EPropertyBagContainerType::Array, EPropertyBagPropertyType::Float }
});

if (auto FloatArrayRef = Bag.GetArrayRef(ArrayName); FloatArrayRef.IsValid())
{
    FPropertyBagArrayRef& FloatArray = FloatArrayRef.GetValue();
    const int32 NewIndex = FloatArray.AddValue();
    FloatArray.SetValueFloat(NewIndex, 123.0f);
}
// The array reference is not valid after the layout of the referenced property bag has changed!

// Set
static const FName FloatSetName(TEXT("Set"));
Bag.AddProperties({
    { FloatSetName, EPropertyBagContainerType::Set, EPropertyBagPropertyType::Float }
});

if (auto FloatSetRef = Bag.GetSetRef(ArrayName); FloatSetRef.IsValid())
{
    FPropertyBagSetRef& FloatSet = FloatSetRef.GetValue();
    FloatSet.AddValueFloat(123.f);
}
#+end_src

#+begin_quote
Don't add/remove properties at runtime, it's quite expensive as it will create new ~UPropertyBag~, reallocate memory, and copy all values over.
#+end_quote

When migrate property from one bag to another, helper function and override for Property can be used.

#+begin_src cpp
// Array of overridden properties.
UPROPERTY(EditAnywhere)
TArray<FGuid> PropertyOverrideGuids;

// The type bag will change to new type,
// and if ID of property is also exist in new bag and PropertyOverrideGuids array,
// the value will copy to new bag.
Bag.MigrateToNewBagInstanceWithOverrides(InBag, PropertyOverrideGuids);
#+end_src
...
Before using, please make sure you really need a new system thread. By default and in the majority of cases parallel processing should be done by *TaskGraph*.

#+begin_src cpp
TUniquePtr<FThread> MyThread;
#+end_src

#+begin_src cpp
{
    if (MyThread && MyThread->IsJoinable())
    {
        // Blocks the current thread，wait for completion of the thread.
        MyThread->Join();
    }

    MyThread.Reset(new FThread(TEXT("ThreadName"), []()
    {
        DoSomething();
    }));
}
...
Unreal “cook” assets for target platform when packaging.

By default, Unreal cook *everything* to prevent user shipping their games with missing asset issues.

To only cook asset you need, edit =Primary Asset Types to Scan= in =ProjectSettings= > =Game - Asset Manager=.

It's common to set =Map='s Cook rule to =Always Cook=, check =Apply Recursively= to also cook assets which the map has referenced, add Maps to =Directories= or =Specific Assets=, and uncheck =Is Editor Only=.

And add any other classes you need cooked as Primary Assets (e.g. runtime-add assets, soft referenced assets, etc.).

** User Settings (Online/Local)

*** Enhanced Input User Settings (Key Bindings)

check =Enable User Settings= in =Project Settings > Enhanced Input > User Settings=

setup =Player Mappable Key Settings= in =InputAction= or =InputMappingContext=, and assign an unique Name for it.

Call ~UEnhancedInputUserSettings::SaveSettings()~ or ~AsyncSaveSettings()~ will create a save game in slot =EnhancedInputUserSettings= (name changeable in project settings) for local player.
...
Run neural networks inside your game.
...
At a basic level, the outer doesn't mean much except that the object has an object path through that outer. EG an actor component has MyCoolWorld.Actor123.MyActorComponent1. Cause it's outered to Actor123 that was spawned in MyCoolWorld.

For most things, this only affects what you can find when using UObject search helpers like finding objects with outer, etc. Sometimes affects replication cause replicated pointers are just pathnames.

There are also several presumptions where things use their outers to find world or similar such outer code where an object is created as part of a system.

But for garbage collection, absolutely nothing matters except that it is in a hard pointer or strong pointer from something else that is safe from garbage collection as well.

Outer for actor components must be always an actor that will own it

By default UObjects get the world from their outer
#+end_quote

Outer is unrelated to the garbage collector, but it does define containment of stuff in stuff - think about making copies of things. if you have an AActor, and it has a pointer variable assigned to point at a certain UObject... if the actor is the outer of that UObject and you duplicate the actor, it will also duplicate the object as if they both came in a box and you're duplicating what's in that box. But if that UObject is actually contained under some other thing in the world, then when you duplicate this actor, instead of also duplicating that object... it won't. The new duplicated actor will still point at the same UObject over there outside of the box instead.
Outer is also most frequently used for world access. children will ask their parent (outer) for its world, and this repeats up the chain until it hopefully hits a world via GetWorld()

when the outermost outer is a package, and the package is deleted, the package will destroy all objects it contained... and it's done by the package, not the garbage collector

*** pointer

You can save or replicate any UObject pointer, they save as names (=MyCoolLevel.MyActorThing_13.MyInventoryComponent.MyInventoryItem_312=).

#+begin_quote
from Authaer

The main point of interest was saving pointers to other objects. 😄 Like ActorA having a pointer to ActorB and being able to retain that at load time without heavy complications.
Also fun fact with Serialize. 😄 You can save stuff from completely unrelated classes, and populate it into the other class as long as their property name is the same. Serialize doesn't care as long as the type and property name is the same.
#+end_quote
...
Player tries to cheat but it doesn't work.
(Preferred, but has limitation)

limiting access to the process memory

**** Secure Boot

Prevent many bootkit-style cheats.

**** TPM

Securely verify that Secure Boot and other settings are enabled.

**** Reflection

Unreal Reflection System contains lot of metadata (like class and function naming) that are very helpful to reverse engineers.

use plain C++ for sensitive code could make reverse engineering more difficult.

**** RPC

Server RPC must verify the quest from client is valid as far as possible.

*** Detection

Player Cheats, later is detected and sanctioned.
...
UObjects aren't really threadsafe without locking for garbage collection, the Audio Thread and similar threads are messy.
#+end_quote
